...
def distance_calc(user):
    ...
    for user in users:
        ...
        for friend in user_frnds:
            ...
            dist_frnd = distance_between_points(user_lat, user_lon, frnd_lat, frnd_lon)
...
producer = KafkaProducer(bootstrap_servers = 'localhost:9092', value_serializer = serializer) #añadimos esta línea para obtener el productor de kafka para la ingesta de datos
...
while True:
    try:
        if keyboard.is_pressed('q'):  # if key 'q' is pressed 
            ...
            print(users)
            print(old)
            producer.send('topic_users', users)
            time.sleep(2)
